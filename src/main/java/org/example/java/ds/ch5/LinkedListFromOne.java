package org.example.java.ds.ch5;
...
public class LinkedListFromOne<T extends Comparable<? super T>> implements ListFromOne<T> {
    ...
    public void swap(int firstPosition, int secondPosition) {
        if (isValidPosition(firstPosition) && isValidPosition(secondPosition)) {
            ...
            throw new IndexOutOfBoundsException("At least one position is invalid for swapping in a list of size "
                    + size + ": " + firstPosition + ", " + secondPosition);
        }
    }

    public ListFromOne<T> getAllLessThan(T target) {
        LinkedListFromOne<T> lessThanList = new LinkedListFromOne<>();
        ...
        while (current != null) {
            ...
            if (currentElement.compareTo(target) < 0) {
                lessThanList.add(currentElement);
            }
