package org.example.java.ds.ch7;
...
public class LinkedMultiset<T> implements Multiset<T> {
    ...
    public int getOccurrencesOf(T element) {
        return getOccurrencesOf(element, head);
    }

    private int getOccurrencesOf(T element, Node current) {
        if (current == null) {
            return 0;
        }

        int countForCurrent = current.data.equals(element) ? 1 : 0;
        return countForCurrent + getOccurrencesOf(element, current.next);
    }
