package org.example.java.ds.ch9;
...
public class SearchSortHomeworkDriver {
    ...
    public static double sortedness(Comparable[] array) {
        ...
        return (double) sortedPairs / (array.length - 1);
    }
    ...
    public static double sortedness(Node<Comparable> node) {
        ...
        return (double) countSortedPairs(node) / countPairs(node);
    }

    private static int countSortedPairs(Node<Comparable> node) {
        if (node == null || node.next == null) {
            return 0;
        }
        ...
        return currentPair + countSortedPairs(node.next);
    }

    private static int countPairs(Node<Comparable> node) {
        if (node == null || node.next == null) {
            return 0;
        }

        return 1 + countPairs(node.next);
    }
